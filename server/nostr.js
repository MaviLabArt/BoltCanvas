...
export function makeCommentProof({ productId, ts } = {}) {
  ...
  const sigBytes = schnorr.sign(hash, keys.seckeyBytes);
...
export function verifyCommentProof({ sig, ts, storePubkey, productId } = {}) {
  ...
  try {
    ...
    const hash = createHash("sha256").update(msg).digest();
    if (!/^[0-9a-f]{128}$/i.test(proofSig)) return false;
    return !!schnorr.verify(Buffer.from(proofSig, "hex"), hash, Buffer.from(key, "hex"));
  } catch {
    return false;
  }
}
